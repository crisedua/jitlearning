...
## The AI-assistant material is strategic on purpose

The corpus in `knowledge/herramientas/` describes Claude, ChatGPT and Gemini at
the level of **what each one is, its advantages and disadvantages, and which
use cases it fits** — not click-by-click instructions. The app used to ship
step-by-step UI tutorials for these products; they were removed because the
interfaces change without notice and stale steps do more harm than none. The
agent's prompt now tells it the same thing: advise on which tool and why,
stay off button-level walkthroughs, and point at the official docs for the
current clicks.
...
knowledge/              Tracked corpus: business material + AI-assistant strategy docs
...
- **The AI-assistant docs describe a fast-moving market.** They are written at
  the strategic level precisely because it ages slowest, but pricing, plans and
  relative capability still shift; each document carries the date it was written
  and points at the official pages for the current detail.
